...
// TESOLA 토큰 보상 계산기 - tesola-staking-plan.md에 따른 구현
...
const REWARD_RATES = {
  ...
  function calculateEstimatedRewards(nftTier, stakingPeriod) {
    ...
    const tier = nftTier?.toUpperCase() || 'COMMON';
   ...
   */
  function calculateEarnedRewards(nftTier, stakingStartDate, currentDate = new Date(), stakingPeriod) {
    // 날짜 객체로 변환
    ...
    return {
      ...
      progressPercentage,
      earnedRewards,
      remainingRewards: calculateEstimatedRewards(nftTier, stakingPeriod).totalRewards - earnedRewards
    };
  }
   ...
   */
  function calculateUnstakingPenalty(nftTier, stakingStartDate, unstakingDate = new Date(), stakingPeriod) {
    // 날짜 객체로 변환
    const startDate = new Date(stakingStartDate);
