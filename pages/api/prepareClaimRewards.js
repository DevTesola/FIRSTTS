...
import { createSerializedTransaction } from '../../shared/utils/transaction';
...
import { STAKE_SEED, USER_STAKING_SEED, POOL_SEED, REWARD_VAULT_AUTHORITY_SEED } from '../../shared/constants/seeds';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 순서가 매우 중요합니다 - Anchor IDL에 정의된 순서와 정확히 일치해야 합니다
    const accounts = [
      ...
      // 리워드 토큰 관련 계정 - 리워드 관련 계정은 반드시 이 순서대로 포함되어야 함
    ...
    // 트랜잭션 직렬화 - 통합 유틸리티 함수 사용
    const serializedTransaction = createSerializedTransaction(
      allInstructions,
      walletPubkey,
      blockhash,
