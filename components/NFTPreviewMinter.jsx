...
export default function NFTPreviewMinter({ mintPrice = "1.5 SOL", onMint, loading = false }) {
      ...
      try {
        ...
        const previewData = [
          // 레전더리 등급
          {
            ...
            name: "SOLARA #001 (Preview)",
            image: "/nft-previews/legendary-1.jpg",
            tier: "Legendary",
            attributes: [
          ...
          },
          {
            ...
            name: "SOLARA #002 (Preview)",
            image: "/nft-previews/legendary-2.jpg",
            tier: "Legendary",
            attributes: [
          ...
          {
            id: "preview-rare-1",
            name: "SOLARA #003 (Preview)",
            image: "/nft-previews/rare-1.jpg",
            tier: "Rare",
            attributes: [
          ...
          {
            id: "preview-rare-2",
            name: "SOLARA #004 (Preview)",
            image: "/nft-previews/rare-2.jpg",
            tier: "Rare",
            attributes: [
          ...
          // 언커먼 등급
          {
            id: "preview-uncommon-1",
            name: "SOLARA #005 (Preview)",
            image: "/nft-previews/uncommon-1.jpg",
            tier: "Uncommon",
            attributes: [
              { trait_type: "Background", value: "Planet Surface" },
              { trait_type: "Base", value: "Bronze" },
              { trait_type: "Special", value: "Dust Storm" }
            ]
          },
          {
            id: "preview-uncommon-2",
            name: "SOLARA #006 (Preview)",
            image: "/nft-previews/uncommon-2.jpg",
            tier: "Uncommon",
            attributes: [
              { trait_type: "Background", value: "Satellite View" },
              { trait_type: "Base", value: "Copper" },
              { trait_type: "Special", value: "Crater" }
            ]
          },
          ...
          {
            id: "preview-common-1",
            name: "SOLARA #007 (Preview)",
            image: "/nft-previews/common-1.jpg",
            tier: "Common",
            attributes: [
          ...
          {
            id: "preview-common-2",
            name: "SOLARA #008 (Preview)",
            image: "/nft-previews/common-2.jpg",
            tier: "Common",
            attributes: [
  ...
  const rarityDistribution = calculateRarityDistribution();
        ...
        <>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap justify-center gap-2">
              ...
              >
                Legendary ({rarityDistribution.legendary})
              </button>
              <button
              ...
              >
                Uncommon ({rarityDistribution.uncommon})
              </button>
              <button
              ...
              >
                Common ({rarityDistribution.common})
              </button>
            </div>
          </div>
                  ...
                  <div className="relative aspect-square">
                    {nft.image.startsWith('/') ? (
                      ...
                      <div 
                        ...
                        style={{ backgroundColor: nft.placeholderBg }}
                      >
                    ...
                    <div className={`absolute top-2 right-2 px-2 py-0.5 rounded text-xs font-bold ${
                      ...
                      nft.tier.toLowerCase() === 'uncommon' ? 'bg-blue-600 text-white' :
                      'bg-green-600 text-white'
                    }`}>
                      {nft.tier}
                    </div>
            ...
            <div className="bg-gray-800 p-4 rounded-lg">
              <div className="h-6 bg-gray-700 rounded-full overflow-hidden">
                <div className="h-full flex">
                  <div 
                    ...
                    style={{ width: `${(rarityDistribution.legendary / previewCollection.length) * 100}%` }}
                    title={`Legendary: ${rarityDistribution.legendary}`}
                  ></div>
                  <div 
                    className="bg-purple-500 h-full" 
                    style={{ width: `${(rarityDistribution.rare / previewCollection.length) * 100}%` }}
                    title={`Rare: ${rarityDistribution.rare}`}
                  ></div>
                  <div 
                    className="bg-blue-500 h-full" 
                    style={{ width: `${(rarityDistribution.uncommon / previewCollection.length) * 100}%` }}
                    title={`Uncommon: ${rarityDistribution.uncommon}`}
                  ></div>
                  <div 
                    className="bg-green-500 h-full" 
                    style={{ width: `${(rarityDistribution.common / previewCollection.length) * 100}%` }}
                    title={`Common: ${rarityDistribution.common}`}
                  ></div>
                </div>
              </div>
              ...
              <div className="mt-2 flex justify-between text-xs">
                <div className="flex items-center">
                  ...
                  <span>Legendary ({Math.round((rarityDistribution.legendary / previewCollection.length) * 100)}%)</span>
                </div>
                <div className="flex items-center">
                  <div className="w-3 h-3 bg-purple-500 rounded-full mr-1"></div>
                  <span>Rare ({Math.round((rarityDistribution.rare / previewCollection.length) * 100)}%)</span>
                </div>
                <div className="flex items-center">
                  <div className="w-3 h-3 bg-blue-500 rounded-full mr-1"></div>
                  <span>Uncommon ({Math.round((rarityDistribution.uncommon / previewCollection.length) * 100)}%)</span>
                </div>
                <div className="flex items-center">
                  <div className="w-3 h-3 bg-green-500 rounded-full mr-1"></div>
                  <span>Common ({Math.round((rarityDistribution.common / previewCollection.length) * 100)}%)</span>
                </div>
              </div>
            </div>
          </div>
