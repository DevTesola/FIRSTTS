...
/**
 * StakingRewards Component - Enhanced for mobile performance
...
const StakingRewards = ({ stats, isLoading }) => {
  ...
  // Get the first NFT for display if none selected (memoized selection)
  useEffect(() => {
    if (!selectedStake && stats?.activeStakes?.length > 0) {
      setSelectedStake(stats.activeStakes[0]);
    }
  }, [stats?.activeStakes, selectedStake]);
  ...
  }, [selectedStake, stats?.stats?.collectionBonus]);

  // Memoized cache bust value that changes only once every 5 minutes
  // This prevents unnecessary image refreshes while still allowing for cache busting
  const cacheBustValue = useMemo(() => {
    // Use the current timestamp but only change it every 5 minutes
    // to prevent unnecessary re-renders for image URLs
    const now = Date.now();
    return Math.floor(now / (5 * 60 * 1000)); // Changes every 5 minutes
  }, []);

  // Memoized image URL processor function
  const processImageUrl = useCallback((stake) => {
    ...
    if (imageUrl && (imageUrl.startsWith('http://') || imageUrl.startsWith('https://'))) {
      try {
        ...
        // 캐시 버스팅 파라미터 추가 (using memoized cache bust value)
        url.searchParams.set('_t', cacheBustValue.toString());
        return url.toString();
      } catch (err) {
        // 추가 캐시 버스팅 파라미터 (using memoized cache bust value)
        return `${imageUrl}${imageUrl.includes('?') ? '&' : '?'}_t=${cacheBustValue}`;
      }
    }
    ...
    if (nftId) {
      ...
      const gatewayUrl = `${IPFS_GATEWAY}/ipfs/${IMAGES_CID}/${formattedId}.png?_t=${cacheBustValue}`;
      return gatewayUrl;
    }
  ...
  // Handle reward claiming with on-chain transaction (memoized with useCallback)
  const handleClaimRewards = useCallback(async () => {
    ...
    if (calculatedValues.claimableRewards <= 0) {
    ...
    try {
      ...
      setSuccessMessage(`Successfully claimed ${calculatedValues.claimableRewards} TESOLA tokens!`);
  ...
  }, [connected, publicKey, calculatedValues.claimableRewards, stats.activeStakes, connection, sendTransaction]);

  // Memoized UI components for performance optimization

  // Reward booster section - memoized to prevent unnecessary re-renders
  const DynamicRewardBooster = useMemo(() => {
    if (isLoading || !boostInfo) return null;
  ...
  // Fallback booster UI when no staking info is available
  const FallbackBoosterUI = useMemo(() => {
    if (isLoading || boostInfo) return null;
  ...
  // Memoized milestones component
  const MilestonesComponent = useMemo(() => {
    if (isLoading || !boostInfo || boostInfo.achievedMilestones.length === 0) return null;
  ...
  // Memoized active stakes list
  const StakesList = useMemo(() => {
    if (isLoading || !stats?.activeStakes?.length) return null;
            ...
            <div className="flex items-center">
              <div className="w-10 h-10 rounded overflow-hidden mr-3 border border-gray-700">
                <ResponsiveImageLoader
                  src={processImageUrl(stake)}
                  alt={stake.nft_name || `Staked NFT #${stake.id}`}
                  placeholder={createPlaceholder(stake.nft_name || `Staked NFT #${stake.id}`)}
                  className="w-full h-full object-cover"
                  lazyLoad={true}
                  priority={false}
                  id={stake.id || stake.mint_address}
                  _source="StakingRewards-thumbnail"
                  _cacheBust={cacheBustValue}
                />
              </div>
              <div>
  ...
  }, [isLoading, stats?.activeStakes, selectedStake, processImageUrl, cacheBustValue]);

  // Memoized empty state
  const EmptyState = useMemo(() => {
    if (isLoading || stats?.activeStakes?.length > 0) return null;
  ...
  }, [isLoading, stats?.activeStakes?.length]);
    ...
    <div className="bg-gradient-to-br from-purple-900/20 to-blue-900/20 rounded-xl p-5 border border-purple-500/20 h-full">
        ...
        <div className="text-center">
          ...
          <div className="text-3xl font-bold text-white">
            {isLoading ? (
              ...
              <>{calculatedValues.claimableRewards} TESOLA</>
            )}
          </div>
        </div>
      </div>

      {/* Dynamic Reward Booster Visualization - Using memoized components */}
      {!isLoading && (
        <>
          {DynamicRewardBooster}
          {FallbackBoosterUI}
        </>
      )}
      ...
      <div className="mb-6">
        <PrimaryButton
          ...
          loading={isClaiming}
          disabled={isLoading || calculatedValues.claimableRewards <= 0 || transactionInProgress}
          fullWidth
          className="py-3"
        >
        ...
        {calculatedValues.claimableRewards <= 0 && !isLoading && (
      ...
      {/* Success or error message - Using memoized component */}
      {StatusMessages}
      
      {/* Milestones Achieved - Using memoized component */}
      {MilestonesComponent}
      ...
      <div className="bg-black/20 rounded-lg p-4">
        ...
        {/* Using memoized components for different states */}
        {LoadingSkeleton}
        {StakesList}
        {EmptyState}
      </div>
