...
// Color constants for consistent tier styling
const TIER_COLORS = {
  LEGENDARY: {
    bg: 'bg-yellow-500',
    bgAlpha: 'rgba(234, 179, 8, 0.2)',
    text: 'rgb(234, 179, 8)'
  },
  EPIC: {
    bg: 'bg-purple-500',
    bgAlpha: 'rgba(168, 85, 247, 0.2)',
    text: 'rgb(168, 85, 247)'
  },
  RARE: {
    bg: 'bg-blue-500',
...
/**
 * StakingAnalytics Component - Enhanced for mobile performance
 * 
...
const StakingAnalytics = ({ stats, unstaked, isLoading, onRefresh }) => {
  ...
  // Memoized button handlers to avoid function recreation
  const handleTimeframeChange = useCallback((newTimeframe) => {
    setTimeframe(newTimeframe);
  }, []);

  const handleChartChange = useCallback((newChartType) => {
    setSelectedChart(newChartType);
  }, []);

  const handleToggleTimeBonus = useCallback(() => {
    setIncludeTimeBonus(prev => !prev);
  }, []);

  const switchToNftsTab = useCallback(() => {
    document.querySelector('[aria-controls="nfts"]')?.click();
  }, []);

  // Memoized UI components to reduce re-rendering
    ...
    return (
      <div className="bg-gray-800/50 border border-purple-500/20 rounded-xl p-6 text-center">
        ...
        <div className="flex justify-center space-x-3">
          ...
          <GlassButton onClick={switchToNftsTab}>
            Stake Your First NFT
          </GlassButton>
        </div>
      </div>
    );
  }, [stats, isLoading, onRefresh, switchToNftsTab]);
          ...
          <div className="w-full sm:flex-1">
            ...
            <div className="flex rounded-lg overflow-hidden border border-gray-700">
              <button 
                ...
                onClick={() => handleChartChange('rewards')}
              >
                Rewards
              </button>
              <button 
                ...
                onClick={() => handleChartChange('breakdown')}
              >
                Breakdown
              </button>
              <button 
                ...
                onClick={() => handleChartChange('earnings')}
              >
                History
              </button>
            </div>
        ...
        <div className="flex justify-center sm:justify-end">
          <label className="inline-flex items-center cursor-pointer">
            <input 
              ...
              onChange={handleToggleTimeBonus}
            />
  ...
  }, [timeframe, selectedChart, includeTimeBonus, handleTimeframeChange, handleChartChange, handleToggleTimeBonus]);
  ...
  }, [analytics.performance, analytics.projectedRewards, timeframe]);

  // Projected rewards chart
  const ProjectedRewardsChart = useMemo(() => {
    if (selectedChart !== 'rewards' || !analytics.projectedRewards) return null;

    // Generate array of important milestone days to show on mobile
    const importantMilestoneDays = [0, 7, 30, 90, 180, 365];
                ...
                <div className="absolute top-0 left-10 sm:left-14 right-0 bottom-0 flex items-end">
                  {analytics.projectedRewards.timeline
                    ...
                    .filter((point) => {
                      ...
                      if (point.day === 0 || 
                          point.day === analytics.projectedRewards.timeline[analytics.projectedRewards.timeline.length - 1].day) {
                        return true;
                      }
                      
                      // Show important milestones
                      return importantMilestoneDays.includes(point.day);
                    })
          ...
          {analytics.projectedRewards && Object.keys(analytics.projectedRewards.byTier).length > 0 ? (
            <div className="space-y-2 sm:space-y-3">
              {Object.entries(analytics.projectedRewards.byTier).map(([tier, amount]) => {
                ...
                // Get tier-specific color from constants
                const bgColor = TIER_COLORS[tier]?.bg || 'bg-green-500';
  ...
  const PortfolioBreakdownChart = useMemo(() => {
    if (selectedChart !== 'breakdown' || !analytics.distribution || !analytics.performance) return null;
        ...
        <div className="bg-gray-900/50 rounded-lg p-4">
          ...
          {analytics.distribution && Object.keys(analytics.distribution.counts).length > 0 ? (
            <div className="h-40 sm:h-48 relative">
              {Object.entries(analytics.distribution.counts).map(([tier, count]) => {
                ...
                // Get tier-specific color from constants
                const tierColor = TIER_COLORS[tier]?.bg || 'bg-green-500';
                
                return (
                  <div key={`dist-${tier}`} className="mb-2">
                    <div className="flex items-center">
                      <div className={`w-3 h-3 rounded-full ${tierColor} mr-2`}></div>
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2 mt-1">
                      <div className={`${tierColor} h-2 rounded-full`} style={{ width: `${percentage}%` }}></div>
                    </div>
                  </div>
                );
              })}
          ...
          {analytics.performance?.bestPerformer ? (
            <div className="flex flex-col items-center">
              ...
              <div className="px-2 py-0.5 rounded-full text-xs mb-3" style={{
                backgroundColor: TIER_COLORS[analytics.performance.bestPerformer.tier]?.bgAlpha || 'rgba(34, 197, 94, 0.2)',
                color: TIER_COLORS[analytics.performance.bestPerformer.tier]?.text || 'rgb(34, 197, 94)'
              }}>
  ...
  }, [selectedChart, analytics.distribution, analytics.performance, stats?.activeStakes?.length]);

  // Earnings history chart
  const EarningsHistoryChart = useMemo(() => {
    if (selectedChart !== 'earnings' || !analytics.earnings || !analytics.performance) return null;
    
    // Memoized earnings labels - show only first, middle, and last on mobile
    const earningsLabels = analytics.earnings.length > 0 ? [
      analytics.earnings[0],
      analytics.earnings[Math.floor(analytics.earnings.length / 2)],
      analytics.earnings[analytics.earnings.length - 1]
    ] : [];
    
    // Memoized paths for the SVG chart
    const chartPaths = useMemo(() => {
      if (!analytics.earnings || analytics.earnings.length === 0) return { areaPath: '', linePath: '' };
      ...
      const areaPath = `
        ...
        ${analytics.earnings.map((point, i) => 
          `L${i * pointStep},${100 - ((point.cumulative / maxValue) * 100)}`).join(' ')}
        L100,100 L0,100 Z
      `;
              ...
              <>
                ...
                <div className="absolute top-0 left-10 sm:left-14 right-0 bottom-0">
                  <svg className="w-full h-full" preserveAspectRatio="none">
                    ...
                    <path d={chartPaths.areaPath} fill="url(#earningsGradient)" />
                    <path d={chartPaths.linePath} fill="none" stroke="#a855f7" strokeWidth="2" />
                    
                    {/* Add data points at fixed intervals for better mobile performance */}
                    {analytics.earnings
                      .filter((_, i) => i % 7 === 0 || i === analytics.earnings.length - 1)
                      .map((point, i) => {
  ...
  }, [selectedChart, analytics.earnings, analytics.performance, chartPaths]);

  // Strategy recommendations component
  const StrategyRecommendations = useMemo(() => {
    if (!analytics.strategy || !analytics.strategy.recommendations.length > 0) return null;
  ...
  if (!stats && !isLoading) return EmptyState;
