 ...
 * NFT 언스테이킹 명령어 생성
 *
 * IDL 정의에 따라 정확히 계정을 구성합니다. (행 1826-1954)
 *
 * @param {PublicKey} owner - NFT 소유자 주소
 * @param {PublicKey} nftMint - NFT 민트 주소
 * @param {PublicKey} poolState - 풀 상태 계정 주소
...
function createUnstakeNftInstruction(
  ...
  // IDL에서 확인된 명령어 discriminator (행 1829-1836)
  const data = Buffer.from(INSTRUCTION_DISCRIMINATORS.UNSTAKE_NFT);

  // IDL 정의에 따른 계정 순서 검증
  if (!accounts.stakeInfo) throw new Error('stakeInfo is required');
  if (!accounts.escrowTokenAccount) throw new Error('escrowTokenAccount is required');
  if (!accounts.escrowAuthority) throw new Error('escrowAuthority is required');
  if (!accounts.userTokenAccount) throw new Error('userTokenAccount is required');
  if (!accounts.userStakingInfo) throw new Error('userStakingInfo is required');

  // IDL에 정의된 순서대로 계정 배열 구성 (행 1838-1954)
  const keys = [
    { pubkey: owner, isSigner: true, isWritable: true },                    // owner
    { pubkey: nftMint, isSigner: false, isWritable: false },                // nft_mint
    { pubkey: accounts.stakeInfo, isSigner: false, isWritable: true },      // stake_info
...
function createClaimRewardsInstruction(
  ...
  const keys = [
    ...
    { pubkey: nftMint, isSigner: false, isWritable: false },      // nft_mint
    { pubkey: accounts.stakeInfo, isSigner: false, isWritable: true },         // stake_info
    { pubkey: accounts.poolState, isSigner: false, isWritable: false }         // pool_state
  ];
