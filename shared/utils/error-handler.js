...
function getErrorMessage(error) {
  // 로깅을 통한 디버깅 지원
  console.log("에러 처리 중:", typeof error, error?.code, error?.message);
  ...
  if (error && error.logs) {
    ...
    for (const log of error.logs) {
      // AccountNotInitialized 오류 (Anchor 특정)
      if (log.includes('AccountNotInitialized') || log.includes('Error Code: 3012')) {
        return '계정이 초기화되지 않았습니다. 초기화 트랜잭션이 필요합니다';
      }
  ...
  if (error && typeof error === 'object') {
    ...
    // 계정 초기화 오류 특별 처리 (Anchor 특정)
    if (error.code === 3012 || (error.message && error.message.includes('AccountNotInitialized'))) {
      return '계정이 초기화되지 않았습니다. 초기화 트랜잭션이 필요합니다';
    }
    ...
    if (error.message) {
      ...
      if (error.message.includes('not initialized') ||
          ...
          error.message.includes('Error Code: 3012')) {
        return '계정이 초기화되지 않았습니다. 초기화 트랜잭션이 필요합니다';
      }
